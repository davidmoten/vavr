...
public abstract class Try<T> implements Iterable<T>, Serializable {
     ...
     * @return {@code Success(callable.call())} if no exception occurs, otherwise {@code Failure(cause)} if an
    ...
    public <U> Try<U> flatMap(CheckedFunction<? super T, Try<? extends U>> mapper) {
     ...
     * <p>
     * If this is a {@code Failure}, it will throw the wrapped cause:
     * {@code new UnsupportedOperationException("Failure.get()", cause)}.
     *
     * @return The computation result if this is a {@code Success}
     * @throws RuntimeException if this is a {@link Failure}
     */
     ...
     */
    @SuppressWarnings("unchecked")
    public Try<T> orElse(Callable<Try<? extends T>> supplier) {
        Objects.requireNonNull(supplier, "supplier is null");
        if (isSuccess()) {
            return this;
        } else {
     ...
     */
    @SuppressWarnings("unchecked")
    public final <X extends Throwable> Try<T> recover(Class<X> exceptionType, CheckedFunction<? super X, ? extends T> recoveryFunction) {
     ...
     */
    @SuppressWarnings("unchecked")
    public <X extends Throwable> Try<T> recoverWith(Class<X> exceptionType, CheckedFunction<? super X, Try<? extends T>> recoveryFunction) {
        Objects.requireNonNull(exceptionType, "exceptionType is null");
        Objects.requireNonNull(recoveryFunction, "recoveryFunction is null");
        if (isFailure()) {
            final Throwable cause = getCause();
     ...
     */
    @SuppressWarnings("unchecked")
    public <U> Try<U> transform(CheckedFunction<? super Throwable, Try<? extends U>> onFailure, CheckedFunction<? super T, Try<? extends U>> onSuccess) {
        Objects.requireNonNull(onFailure, "onFailure is null");
        Objects.requireNonNull(onSuccess, "onSuccess is null");
        try {
            return isSuccess()
    ...
    private static final class Failure<T> extends Try<T> implements Serializable {
        ...
        public T get() throws RuntimeException {
            throw new RuntimeException("Failure.get()", cause);
        }
